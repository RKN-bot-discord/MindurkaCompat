...
public enum EditorTool {
    ...
    castleRoomPlace(KeyCode.q) {
        ...
        public void toolOptions(Table table) {
            ...
            table.field(String.valueOf(MVars.toolOptions.blockCostFor(MVars.toolOptions.selectedBlock)), text -> {
                MVars.toolOptions.blockCost = Integer.parseInt(text);
            }).growX().left().row();
            table.label(() -> "@rules.status.invincible").left().pad(6).row();
            table.field(String.valueOf(MVars.toolOptions.invincible || MVars.toolOptions.selectedBlock instanceof Turret), text -> {
        ...
        public void touched(ToolContext ctx, int x1, int y1, int x2, int y2) {
            if (ctx instanceof PreviewToolContext) {
                int size = MVars.toolOptions.selectedBlock.size;
                int bx = x2 - size / 2;
                int by = y2 - size / 2;
            ...
            if (ctx.isErase()) {
                ...
                if (ctx.isLayer()) {
                    int bx = x1;
                    int by = y1;
                    Castle.CastleBlock b = new Castle.CastleBlock(MVars.toolOptions.selectedBlock, bx, by, MVars.toolOptions.blockCost, MVars.toolOptions.invincible || MVars.toolOptions.selectedBlock instanceof Turret);
    ...
    castleMinerPlacer(KeyCode.w) {
        ...
        public void toolOptions(Table table) {
            ...
            table.field(String.valueOf(MVars.toolOptions.selectedBlock), text -> {
                MVars.toolOptions.minerDrill = MVars.toolOptions.selectedBlock;
            }).growX().left().row();
        ...
        public void touched(ToolContext ctx, int x1, int y1, int x2, int y2) {
            ...
            if (ctx.isErase()) {
                ...
                if (ctx.isLayer()) {
                    int bx = x1;
                    int by = y1;
                    Castle.CastleMiner b = new Castle.CastleMiner(MVars.toolOptions.selectedBlock, bx, by, MVars.toolOptions.minerCost, MVars.toolOptions.minerAmount, MVars.toolOptions.minerInterval,MVars.toolOptions.selectedItemCastle);
