...
public class UnitSelectDialog extends BaseDialog {
    Boolf<UnitType> filter;
    Cons<UnitType> set;
    WeakReference<Table> filteredContent;
    UnitType chosen;

    public UnitSelectDialog(String title) {
        super(title);
    ...
    private void rebuildItems(String filter) {
        ...
        for (UnitType unit : Vars.content.units()) {
            if (unit.uiIcon == null) continue;
            if (!unit.name.contains(filter) || !this.filter.get(unit)) continue;

            ImageButton button = new ImageButton(Tex.whiteui, Styles.clearNoneTogglei);
            button.getStyle().imageUp = new TextureRegionDrawable(unit.uiIcon);
            button.resizeImage(32f);
            button.setChecked(unit == chosen);
            button.clicked(() -> {
                set.get(unit);
                hide();
            });
    ...
    public void show(Boolf<UnitType> filter, Cons<UnitType> set, UnitType chosen) {
        this.filter = filter;
        this.set = set;
        this.chosen = chosen;
        show();
