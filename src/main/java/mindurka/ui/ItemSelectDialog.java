...
public class ItemSelectDialog extends BaseDialog {
    ...
    public ItemSelectDialog(String title) {
        super(title);
    ...
    private void rebuildItems(String filter) {
        ...
        for (Item item : Vars.content.items()) {
            if (item.uiIcon == null) continue;
            if (!item.name.contains(filter) || !this.filter.get(item)) continue;
    ...
    public void show(Boolf<Item> filter, Cons<Item> set, Item chosen) {
        this.filter = filter;
        this.set = set;
        this.chosen = chosen;
        show();
